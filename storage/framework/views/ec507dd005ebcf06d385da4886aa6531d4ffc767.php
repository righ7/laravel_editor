    ...
    let goodsShow = {
        ...
        template:`
            <div style="margin: 2px;">
                        ...
                        <div @click="goodsActive(index)">
                            <div class="card-content" style="width: 165px;margin: 10px auto;">
                                <div style="position: relative;">
                                    <img @click="addtag(goods,index)" src="<?php echo e(asset('images/wujiao.png')); ?>" :style="{ display: tagsSetShow }" style="cursor: pointer;z-index:99;width: 20px;height: 20px;position:absolute;top:0px;right:23px;"></img>
                                    <img @click="feedimgclk(goods)" src="<?php echo e(asset('images/feedback.png')); ?>" :style="{ display: errorGoodShow }" style="cursor: pointer;z-index:99;width: 20px;height: 20px;position:absolute;top:0px;right:0;"></img>
                ...
                <modal
                        title="商品反馈"
                        v-model="feedgoodsmodal"
                        ok-text="提交"
                        @on-ok="getFeedback"
                        width="300"
                        class-name="vertical-center-modal">
                    <radio-group v-model="feedgoodsslect" vertical>
                        <Radio label="产品重复">
                            <span>产品重复</span>
                        </Radio>
                        <Radio label="价格过高">
        ...
        methods: {
            getGoodsData(page = 1) { // 获取商品数据
                ...
                axios.get("<?php echo e(route('goods_tag.get_goods_tags')); ?>", {
                    params: {}
                })
                    .then(function (response) {
                        if (response){
                            if (response.data.tagRoot==true){
                                that.tagsSetShow='bolck';
                            }
                            if (response.data.feedbackRoot==true){
                                that.errorGoodShow='bolck';
                            }
                        }
                    });
            },
            ...
            findGoodsSame(goods){
                ...
                // 请求公共分组/自有分组下商品数量
                axios('<?php echo e(route('goods_group/find_goods_in_group')); ?>',{
                    params: {
            ...
            //商品反馈
            feedimgclk(goods){
                this.feedgoodsmodal=true;
                this.selectfeed=goods;
            },
            getFeedback(){
                let that=this;
                layer.confirm("确定要提交反馈？", {
                    btn: ['确认','取消']
                }, function(index){
                    $.ajax({
                        url: "<?php echo e(route('goods_tag.add_feed_back')); ?>",
                        type:'GET',
                        dataType:'json',
                        data: {
                            content:that.feedgoodsslect,
                            goods_title:that.selectfeed.title,
                            goods_url:that.selectfeed.goods_url,
                            goods_id:that.selectfeed.goods_id
                        },
                        success:function(json){
                            if(json.errorCode == false){
                                that.$Message.success('已提交,可在问题反馈内查看');
                                layer.close(index);
                                return false;
                            }else{
                                that.$Message.error('服务器异常！请稍候再试');
                                layer.close(index);
                                return false;
                            }
                        },
            ...
            // 提取店铺ID / 商品ID
            extractTailNumber(str = '') {
                if(str == '') return '';
                let regexp = /[0-9]+/g;
                let num = str.match(regexp);
                return num[num.length - 1];
            }
        },
        watch: {
            'search.goods_id': function(n,o){
                this.search.goods_id = this.extractTailNumber(n);
            },
            'search.shop_id': function(n,o){
                this.search.shop_id = this.extractTailNumber(n);
            },
        }
    };
</script>