...
class MyPublishController extends Controller
{
     ...
     * @throws \Exception
     */
    public function changeArticleStatus(HRequest $request, $id)
    {
        $status = $request->status ?? 3;

        if($status == 3){ // 提交审核校检 & 内容标记
            $rs = CArticle::dataCheck($id);
            if($rs != 'true'){
        ...
        if($status == 2){ // 有可能是驳回，删除掉分发表数据
            PlatformArticleDistribute::where('status',2)->orWhere('is_get', 0)->where('article_id',$id)->delete();
        }
        ...
        if($status == 3){ // 提交审核、免审核
            CArticle::avoidAudit($id);
        }
    ...
    protected function grid()
    {
        ...
        $grid->tools(function ($tools) use ($status) {
            ...
            $tools->append(new ArticleStatus(1));
        });
        ...
        $grid->type('文章类型')->sortable()->display(function () {
            if ($this->type == 1)
                return "<div class=\"ivu-tag ivu-tag-blue ivu-tag-checked\"><span class=\"ivu-tag-text ivu-tag-color-white\">图集</span></div>";
            if ($this->type == 2)
                return "<div class=\"ivu-tag ivu-tag-cyan ivu-tag-checked\"><span class=\"ivu-tag-text ivu-tag-color-white\">专辑</span></div>";
